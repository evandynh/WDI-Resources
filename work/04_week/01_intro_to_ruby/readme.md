...
- Describe the different types of variables (local, instance, constant) in Ruby & when to use them
...
Now that you've gotten your feet wet with HTML, CSS, and JavaScript, you've got a taste of what it's like to start building for the web – and we're about to kick it up a notch.

Originally, the web was meant just as a place for documents – HTML pages that linked to each other, that was it.

But as developers started creating more and more pages, and desiring more and more interactivity with those pages, we got to a point historically where developers started writing languages that created HTML for us. That was where the concept of web development frameworks came from, and undoubtedly one of the most prolific has been Ruby on Rails – one of the first frameworks to use the language of Ruby to build web applications.

We're not going to jump right into Rails immediately; we're going to build up to it over the next few days, and make sure you have an understanding of the concepts that go into building it first.
...
As we experiment with Ruby syntax, you should follow along and try things yourself. Do what I do, but feel free to mess around and try your own little experiments too.

We're going to use IRB, our Interactive Ruby Shell, so we can type some Ruby commands and see exactly what happens in real time, and you can follow along and code.

Open up your terminal, and, from anywhere, type `irb`
![IRB in Terminal](https://cloud.githubusercontent.com/assets/25366/8418624/52d220f6-1e69-11e5-8e3f-bcc902944475.png). Alternatively, you can type `pry` to use pry, which is like irb, but has some syntax highlighting that you might like.
...
> In the end you'll find you have an appreciation for both JS with semicolons and JS and Ruby without them, but for now let's relish forgetting the ';'
...
- `{key: 'value' }` is called a **Hash**, but works the same as a JS object
...
Unlike JavaScript, Ruby has both an Integer and a Float class. This creates some interesting results! Let's take a look in IRB:
...
We looked at string interpolation in JS, which only works inside backticks and looks like this:

```js
`${first} ${last}`
```

String interpolation in Ruby works inside regular old quotation marks.
...
An instance variable (lower_snake_case) is a variable that is defined as an instance of an object. That's not meant to be a fancy term - an instance is just an example of an object, one thingy in the great world of things.

```ruby
@some_variable = "cats" # "cats"
...
**Hint:** Despite being slightly different, at this point you can think of `puts` as similar to `console.log`.
...
Use what you just learned about Ruby data types, methods and string interpolation, hop in ```irb```, and get through as many of the following tasks as you can:
...
We'll get to see a lot more of Ruby over the next couple days, and the next couple weeks.

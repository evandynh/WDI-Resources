...
![Frodo ponders the power of the ring.](starter_code/images/frodo.jpg)

Using our new-found knowledge of jQuery, create a function for each of
the following steps to practice JavaScript and DOM Manipulation with jQuery. 

### Setup

Copy the starter code out of the class repo and into your workspace.

Check out [`js/fellowship.js`](starter_code/js/fellowship.js). 
This is where you will work.

Starter data and assets(character and land names) has been provided for you.  You should only have to make changes in `fellowship.js`.  Note that `data.js` is loaded first in our html, so all of its data is available to `fellowship.js`.

### Directions

1.  Define and invoke the functions outlined below inside of
    [`js/fellowship.js`](starter_code/js/fellowship.js).
...
function makeBuddies() {
  ...
  //     data.js.
  // 4   Give each li tag a class of "buddy", set the text of each li
  //     to one of the Buddies in the array and append them to
  //     "ul#buddies".
  // 5.  Insert the aside tag as a child element of the article.land 
...
function forgeTheFellowShip() {
  ...
  //     article.land for "Rivendell". Append a list to it.
...
2.  When the user "clicks" on the Ring, run the function
    `nazgulScreech` (found in [`js/data.js`](starter_code/js/data.js))

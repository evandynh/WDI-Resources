...
Recently, Angular introduced a new official router, called Component Router. This router only works for Angular 1.5+.

**Our ultimate goal is to build out two pages – a main Todo list and an Archive page for all the Todos we've completed.**

Let's walk through it.

## Seven Steps to UI-Router - Codealong (35 mins)
...
During installfest, we added a simple command `server` - using commands already built into your computer - that runs a http-server.  Navigate to our starter app folder and run:
...
> Answer: `<li ng-repeat="todo in todosVm.remainingTodos()">`
...
**What other states would be good to add to your app?** Try adding an about page to start, or even crazier, maybe adding an extra state to be able to _edit_ a todo. Take the next 5 minutes to try, and we'll be here to help if you need it.
